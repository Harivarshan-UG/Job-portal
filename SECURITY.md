...
1. **Keep `.env.example` updated** - When you add new environment variables, update the example file (without real values)
2. **Document all variables** - Add comments in `.env.example` explaining what each variable does
3. **Use different values for dev/prod** - Never use development credentials in production
4. **Rotate secrets regularly** - Change production secrets periodically
5. **Use environment-specific files** - Consider `.env.development`, `.env.production` for clarity
